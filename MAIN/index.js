var connected_flag = 0;       //tell if the web is connected
...
$(document).ready(function () {
  ...
  $("#Login_block :input").prop("disabled",true); //----disable the form, incase accidently acviateit
...
function checkOnLine(){   
    if (typeof(navigator.onLine)!="undefined"){
      setTimeout(() => checkHandler(), 3000);   //----delay open the app       
    }else{   
...
function checkHandler(){   
  ...
  if(status){   //----the client is connect to the server
    //$("#refresh").text("online");   
    connected_flag = 1;
    ...
    $("#refresh").append($("<div></div>").css({
      ...
      "opacity": "0.6",
      "border-radius": "5vw" 
    }).hide().fadeIn(600)); //----imformation background
    $("#refresh").append($("<p></p>").text("Please connect to Internet").css({
      "position": "fixed",
      "top": "40vh",
      "left": "15vw",
      "width": "70vw",
      ...
      "textAlign": "center",
      "opacity": "0"
    }).animate({opacity: '1'}, 1000));  //----imformation
